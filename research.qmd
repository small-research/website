...
def print_paper_table():
    ...
    # Renomeia colunas
    paper_table = paper_table.rename(columns={'Publication year': 'year', 'Proceedings title': 'p.title'})
    
    # Filtra e ordena os dados
    paper_table = paper_table[paper_table['year'] > 2014].sort_values(by='year', ascending=False)
    
    # Cria a coluna de conteúdo formatada em HTML
    paper_table['content'] = (
        paper_table['Authors'] + 
        ". <b>" + paper_table['Title'] + 
        "</b>. " + paper_table['Journal'].fillna('') + 
        paper_table['p.title'].fillna('') + "."
    )
